...
exports.fetchAndSaveJobs = async () => {
                    try {
                                        ...
                                        const newJobs = feed.items.map((item) => {
                                                            ...
                                                            const applyLinkMatches = description.match(
                                                                                /(?:Apply\s*Here\s*:\s*|Apply\s*Link\s*:\s*)(https?:\/\/[^\s]+)/gi
                                                            );
                                                            const hourlyRangeMatch = description.match(
                                                                                /(?:Hourly\s*Rate|Hourly\s*Range)\s*:\s*([\d\s\$\-]+)/i
                                                            );
                                                            ...
                                                            const skillsMatch = description.match(
                                                                                /(?:Skills|Required Skills)\s*:\s*([^\n]+)/i
                                                            );

                                                            const formattedDescription = description
                                                                                .replace(
                                                                                                    /(?:Hourly\s*Rate|Hourly\s*Range|Budget|Category|Country|Skills|Required Skills|Apply\s*Here\s*:\s*https?:\/\/[^\s]+):[^\n]*\n?/gi,
                                                                                                    ""
                                                                                )
                                                                                .trim();
                                                            const uniqueSkills = [
